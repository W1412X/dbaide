...
class SemanticClarifier:
    ...
    def analyze(
        ...
        system = (
            ...
            "values below ARE the candidate set — they are known to you, so you must NEVER ask an "
            "open 'which field/column/table/value?' question when the candidates are sitting in "
            "that schema. For ANY question of the form 'which column', 'which table', or 'which "
            "value qualifies', you MUST populate 'options' with the actual matching column names, "
            "table names, or observed values from the data below (list every plausible candidate, "
            "most-likely first, so the user just picks). Example: for 'how many sane employees', do "
            "NOT ask 'which field indicates sane?' — instead ask 'Which column identifies a sane "
            "employee?' with options drawn from the employees table's columns (e.g. status, "
            "mental_state, is_active …).\n"
            "- Leave 'options' empty ONLY for a question whose answer genuinely is NOT in the "
            "schema/values (e.g. a timezone, an external business rule). If the answer is a "
            "column/table/value you can see, options MUST be non-empty.\n"

...
class AskAgentLoop:
    ...
    def _decide(self, state: LoopState, transcript: list[str]) -> dict[str, Any]:
        ...
        system = (
            ...
            "- clarify_semantics: before generate_sql, it inspects the resolved schema (and the real observed values of its columns) and surfaces EVERY genuinely-uncertain interpretation as a question for the user — open-ended, whatever applies to this question (value encodings, which column, time/timezone, metric definition, scope, …). If it pauses, wait for the reply; if it returns clear, proceed. You may also call it again later if new uncertainty appears (e.g. after inspecting more data). Do not substitute your own guess for it.\n"
            ...
            "- For a column's value ranges / null rate / distinct / length, call column_stats (pick only the metrics you need); for a whole-table overview omit columns.\n"

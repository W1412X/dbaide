...
- 📈 **交互看板。** 把任意一次分析变成可保存的**参数化看板** —— 筛选控件、KPI 卡片、
  图表与明细表，全部由只读 SQL 驱动。已保存看板在 **看板** 标签页打开（默认只读查看），
  可用自然语言优化，并以标签页形式并排打开多个。
...
分析出有价值的结果？在任意回答下打开 **更多 ⋯** 菜单，选 **AI 生成交互看板…**。
构建 Agent 会把这次分析变成一个**参数化看板** —— 筛选控件、KPI 卡片、多张图表和一张
明细表，全部基于只读 SQL，走和 Agent 同一套护栏。生成时会在 **看板** 模式下用一个独立
标签页展示构建过程。

已保存的看板在顶部 **看板** 标签页：

- 🖼️ **看板库。** 每个看板一张卡片（名称、图表数、连接、日期），带 **打开** / **删除**。
- 👁️ **默认只读查看。** 打开看板只显示筛选和图表；选好筛选值点 **应用** 重新查询。
  模型选择与输入框不会碍事 —— 那些属于编辑。
- ✍️ **编辑即优化。** 点 **编辑** 展开模型选择和优化输入框，用自然语言描述改动
  （如「月份改成日期范围、加个按品类的饼图」），看板就地重建。
- 🗂️ **标签页。** 多个看板以可关闭标签页并排打开；角上的 **看板库** 按钮返回画廊。
